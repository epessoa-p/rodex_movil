    ...
    return Scaffold(
      ...
      body: async.when(
        ...
        data: (d) {
          ...
          _selected.removeWhere((id) => !d.pending.any((o) => o.orderId == id));
      ...
      child: Row(
        children: [
          ...
          Column(
            ...
            children: [
              Text(
                'Comisión ${_trim(d.commissionRate)}%',
                style: const TextStyle(fontSize: 12),
              ),
              Text(
                'Pagado ${money(d.paidTotal)}',
  ...
  Future<void> _shareReceipt(
    ...
    try {
      ...
      await Printing.sharePdf(
        bytes: bytes,
        filename: 'comprobante-${pago.id}.pdf',
      );
    } catch (e) {
      if (mounted) {
...
class _PaySheetState extends ConsumerState<_PaySheet> {
  late final TextEditingController _amount = TextEditingController(
    text: widget.commissionTotal.toStringAsFixed(2),
  );
  ...
  Future<void> _save() async {
    ...
    try {
      await ref
          .read(mechanicPaymentsRepositoryProvider)
          .pay(
            mechanicId: widget.mechanicId,
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Text(
                '${widget.workOrderIds.length} OT(s) · comisión',
                style: const TextStyle(color: Colors.black54, fontSize: 13),
              ),
              Text(
                money(widget.commissionTotal),
                style: const TextStyle(color: Colors.black54, fontSize: 13),
              ),
            ],
          ),
