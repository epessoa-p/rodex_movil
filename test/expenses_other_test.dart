...
void _payrollTests() {
  testWidgets('Personal → hoja de pago con selección de período (Día/Semana/Quincena/Mes/Otro)',
      (tester) async {
    await tester.pumpWidget(ProviderScope(
      overrides: [
        authControllerProvider.overrideWith((ref) => _FakeAuth(ref)),
        expensesOverviewProvider.overrideWith((ref) async => ExpensesOverview(
              monthTotal: 0, payrollMonthTotal: 0, services: [], recent: [],
              personal: [
                PersonalRow(id: 4, name: 'Juan Pérez', cargo: 'Mecánico',
                    lastPayment: PaymentStamp(date: '2026-08-30', amount: 1800, period: 'Quincena')),
              ],
            )),
      ],
    ...
    final quincena = tester.widget<ChoiceChip>(find.widgetWithText(ChoiceChip, 'Quincena'));
...
void _toastTests() {
  testWidgets('Con la hoja abierta, el aviso de validación se ve (toast arriba, no SnackBar tapado)',
      (tester) async {
    await tester.pumpWidget(ProviderScope(
      overrides: [
        authControllerProvider.overrideWith((ref) => _FakeAuth(ref)),
    ...
    expect(find.byType(SnackBar), findsNothing);           // ya no se usa el SnackBar tapado
  ...
  test('titleForCode mapea los códigos de negocio a títulos claros', () {
    expect(AppToast.titleForCode('insufficient_balance'), 'Fondos insuficientes');
    expect(AppToast.titleForCode('no_open_session'), 'Caja cerrada');
    expect(AppToast.titleForCode('amount_exceeds_balance'), 'Monto mayor al saldo');
