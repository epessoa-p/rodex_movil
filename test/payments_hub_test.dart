...
final _overview = ExpensesOverview(
  ...
  services: [
    RecurringService(
      ...
      name: 'Luz (CRE)',
      type: 'basico',
      typeLabel: 'Servicio básico',
      defaultAmount: 350,
      paidThisMonth: PaymentStamp(date: '2026-09-05', amount: 350),
    ),
    RecurringService(
      id: 2,
      name: 'Internet (Tigo)',
  ...
  recent: [
    ExpenseMovement(
      date: '2026-09-05',
      description: 'Servicio: Luz (CRE) · Sep 2026',
      amount: 350,
      source: 'Caja',
    ),
  ],
...
void main() {
  ...
  testWidgets(
    'Solo cash.operate: Personal y Gastos, sin Mecánicos ni Proveedores',
    (tester) async {
  ...
  testWidgets('Proveedores agrupa por proveedor con subtotal y total', (
    ...
    await tester.pumpWidget(
      _app(const ['accounts-payable.view', 'cash.operate']),
    );
    await tester.pumpAndSettle();
    ...
    expect(
      find.textContaining('2 facturas · la más antigua hace 53 días'),
      findsOneWidget,
    );
  ...
  testWidgets(
    'Gastos: servicio pagado y sin pagar muestran su estado del mes',
    (tester) async {
      await tester.pumpWidget(_app(const ['cash.operate']));
      await tester.pumpAndSettle();
      ...
      expect(
        find.widgetWithText(FloatingActionButton, 'Otro gasto'),
        findsOneWidget,
      );
    },
  );
