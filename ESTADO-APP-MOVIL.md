...
- ✅ **Foto del producto** (2026-09-25, `products.edit`): la **miniatura del listado** de Inventario es el atajo — un toque abre *Tomar foto · Elegir de la galería · Quitar foto* y la sube al momento, refrescando solo esa fila (con indicador sobre la miniatura mientras sube). El mismo campo está en la **hoja de editar** (miniatura 64 px + "Agregar/Cambiar foto", se guarda al elegirla, no espera al botón Guardar) y en la **ficha** ("Cambiar foto" bajo la galería, o marcador "Agregar foto" si no tiene). La imagen se reduce a 1280 px / calidad 80 antes de subir y la miniatura se decodifica con `cacheWidth`. Endpoints `POST /products/{id}/photo` (multipart `photo`, reemplaza la principal y borra el archivo anterior) y `DELETE /products/{id}/photo` (asciende la siguiente si hay). `GET /products/{id}` devuelve también `image_url`. Widget `ProductThumb` + `pickAndUpdateProductPhoto` (`features/products/product_photo.dart`). Test `product_photo_test.dart`.
...
- ✅ **Listado de ventas: estado en español, con color y saldo** (2026-09-22): la API devuelve `paid/partial/pending` y el badge los mostraba crudos; ahora se traducen (Pagada verde · Parcial naranja · Pendiente rojo, acepta también los valores en español de las OTs) y cada fila a crédito muestra el chip **Crédito** y **Saldo Bs X** en rojo cuando queda por cobrar. Igual corregido en el recibo de venta tamaño carta. Test `sales_history_status_test.dart`.
- ✅ **Reportes: Finanzas e Inventario** (2026-09-21): el hub Reportes tiene Análisis · **Finanzas** · **Inventario**. *Finanzas* (`finance_report_screen.dart`, ruta `/reports/finance`; `/income-statement` es alias) comparte un selector de período (`report_period.dart`: Esta semana · Semana ant. · Este mes · Mes ant. · Todo · Rango) entre tres tabs: **Resultados** (el estado de resultados, ahora `IncomeStatementView`), **Movimientos** (KPIs ingresos/egresos/balance, filtro ingresos/egresos y por sucursal, barras por categoría, lista agrupada por día; máx. 300, avisa si hay más) y **Cierres** (por sesión: caja·sucursal, quién abrió/cerró, apertura, ingresos, egresos, esperado, contado y **diferencia** exacto/faltante/sobrante; abiertas primero). Endpoint `GET /reports/cash?preset=|from&to&branch_id` (`cash-registers.view`, plan cash). *Inventario* (`inventory_report_screen.dart`, `/reports/inventory`): chips de almacén, 5 KPIs (productos, unidades, valor a costo, valor a venta, ganancia potencial), por categoría con barras y stock bajo. Endpoint `GET /reports/inventory?warehouse_id` (`products.view`, plan inventory) sobre `StockValuationService` (compartido con la web). Tests `finance_report_test.dart`. **Cuentas** (`accounts_report_screen.dart`, `/reports/accounts`): tabs **Por cobrar** (ventas a crédito + OTs entregadas con saldo) y **Por pagar** (compras con saldo); por defecto "Todo" = todo lo pendiente, el período acota por fecha del documento. Total y cantidad, **antigüedad** 0–30 / 31–60 / +60 días, filas ordenadas por saldo con fecha, vencimiento (venció/vence), días y "de total"; toque abre la OT o la compra; en Por cobrar botón WhatsApp con recordatorio de saldo prellenado. Endpoints `GET /reports/receivables` (`cash-registers.view|sales.view|workshop.view`) y `GET /reports/payables` (`accounts-payable.view|purchases.view|cash-registers.view`). Test `accounts_report_test.dart`.
- ✅ **Color por módulo en hubs y tabs** (2026-09-18): paleta única `lib/core/module_colors.dart` (`ModuleColors`: ventas verde, productos índigo, categorías naranja, marcas púrpura, modelos gris-azul, orígenes cian, OTs púrpura oscuro, agenda rosa, servicios teal, mecánicos azul, compras marrón, OCs naranja fuerte, proveedores gris-azul, personal azul, gastos rojo, tesorería teal…; helpers `soft()`/`onSoft()`). Nuevo `lib/core/hub_nav_bar.dart` (`HubTab`, `HubNavBar`, `HubAccentBar`): en Taller, Inventario, Compras y Pagos cada tab muestra su ícono en su color, la píldora y la etiqueta activa toman ese color y una franja de 3 px **justo encima de la barra de tabs** cambia (animada) al color del tab. El color "sigue" al proceso: FAB tonal del mismo color en OTs, Agenda, Servicios, Mecánicos, Productos, catálogos y Compras; avatares de servicios/mecánicos/catálogos; `CatalogType.color`. Ficha del cliente: tabs Ventas/OTs/Vehículos/Citas/Alquileres con ícono y contador en su color e indicador del color activo. Home, el **menú lateral** (cada ítem con su ícono sobre un cuadro de su color: Dashboard, Ventas, Inventario, Clientes, Taller, Pagos, Compras, Tesorería, Reportes, Ajustes) y `_Accent` de la OT leen de la misma paleta. Tests `hub_nav_bar_test.dart` + aserción en `client_detail_test.dart`.
- ✅ **Modificar una OT** (2026-09-25): en el detalle, **lápiz** (OT en curso) abre una hoja para corregir kilometraje, combustible, falla reportada, "recibido con" y notas; si la OT no tiene cliente (servicio rápido) permite asignarlo y elegir su vehículo o dejar el texto libre (`work_order_edit_sheet.dart`, `PUT /work-orders/{id}`, permiso `workshop.edit`). Y en una OT **entregada** aparece **Reabrir para corregir**, habilitado solo mientras **la caja de ese cobro siga abierta**: anula el cobro (borra el movimiento de caja y los pagos), **devuelve los repuestos al stock** con su entrada de kardex, revierte los puntos de fidelización y deja la OT en "terminada" para corregirla y cobrarla de nuevo (`POST /work-orders/{id}/reopen`; 422 `reopen_blocked` con el motivo: caja cerrada, comisión ya pagada, crédito o no entregada). Misma lógica en la web (botón en la vista de la OT) vía `App\Services\Workshop\WorkOrderReopenService`. Test `work_order_edit_reopen_test.dart`.
- ✅ **Servicio rápido (OT al paso)** (2026-09-18): en Taller → OTs hay **dos botones flotantes**: ⚡ **Servicio rápido** (arriba) y **Nueva OT** (recepción completa). La pantalla `quick_service_screen.dart` elige servicios del catálogo y **repuestos del inventario** (selector de productos + cantidad/precio; se descuentan del stock con kardex al cobrar, vía `deliverWorkOrder`) (misma hoja `AddServiceSheet`, ahora en `service_pick_sheet.dart`), mecánico opcional, **cliente y vehículo opcionales** (vehículo en texto libre o de la lista del cliente), forma de pago, descuento, notas y, si no hay caja abierta, el aviso de que hace falta para cobrar (con caja abierta no se muestra saldo: el dinero entra); "Cobrar Bs X y cerrar" crea la OT **ya entregada y pagada** (cobro a caja, comisión del mecánico, puntos si hay cliente) y abre el detalle para compartir el recibo. Endpoint `POST /work-orders/quick` (`workshop.create`, 422 `cash_session_required` sin caja). El listado muestra "Cliente de paso · ⚡ rápida"; `WorkOrder.isQuick`. **SQL:** `20260918_work_orders_quick.sql`. Tests `quick_service_test.dart`. **Nota (2026-09-18):** la pantalla salía en blanco en el celular porque el tema da ancho mínimo infinito a `FilledButton` y el botón "Agregar" estaba en un `Row`; ahora se acota con `styleFrom(minimumSize)` y **todos los widget tests usan `AppTheme.light()`** para reproducir el tema real.
- ✅ **Hub Inventario + paginación + catálogos** (2026-09-18): el menú "Productos" pasa a **Inventario** (`/products` → `InventoryHubScreen`) con tabs Productos · Categorías · Marcas · Modelos · Orígenes (cada uno gateado por `*.view`). Los tabs se construyen **al abrirlos por primera vez** (IndexedStack con placeholders): entrar solo carga productos. **Productos paginado**: `GET /products?page=&per_page=30` devuelve `data` + `meta`; la lista muestra 30 por página con **paginador al pie** (barra inferior con línea de progreso, ⏮ ‹ píldora "Página X de Y" › ⏭ en el índigo de Inventario; la píldora abre un selector para saltar a cualquier página) y contador "1–30 de 512 productos"; si el backend no envía `meta`, pagina localmente. Búsqueda con espera de 450 ms (`PosRepository.productsPage`, `ProductPage`). **Catálogos**: API `GET/POST /catalogs/{categories|brands|origins|moto-brands|moto-models}` y `PUT /catalogs/{type}/{id}` (`Api\CatalogController`, permisos `<módulo>.view/create/edit`; el alta también con `products.create|edit`; reutiliza/reactiva si el nombre ya existe, 422 `catalog_name_taken` al editar con nombre duplicado). Móvil: `features/inventory/` (`catalogs_repository.dart`, `catalog_screen.dart` genérico con buscador/alta/edición/activo; modelos con marca de moto, cilindrada, año y precio sugerido, y alta rápida de marca de moto). **Categoría/Marca en el producto** (alta y edición): `CatalogPickerField` abre una hoja con buscador; si lo escrito no existe ofrece **"Crear «X»"** y lo deja seleccionado. Tests `inventory_hub_test.dart` (tabs perezosos, paginación, crear desde el selector).
...
- **Inventario (plan:inventory):** `POST /products/{id}/stock-adjust` (ajuste de stock, `products.edit`), `POST /products` + `GET /product-form-data` (alta rápida, `products.create`), `POST|DELETE /products/{id}/photo` (foto principal, `products.edit`).
